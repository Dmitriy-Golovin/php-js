...
class Plot {
    ...
    public static function plots_list_users($user_id) {
        // vars
        $items = [];
        // info
        $q = DB::query("SELECT plots.plot_id
            FROM plots
            LEFT JOIN users_plots ON users_plots.plot_id = plots.plot_id
            WHERE users_plots.user_id = " . $user_id . " ORDER BY plot_id;") or die (DB::error());
        while ($row = DB::fetch_row($q)) {
            $items[] = $row['plot_id'];
        }
        // output
    ...
    public static function select_plots_list($user_id) {
        // vars
        // field status ???
        $items = [];
        // info
        $q = DB::query("SELECT plot_id,
            IF((SELECT user_plot_id FROM users_plots WHERE users_plots.plot_id = plots.plot_id AND user_id = " . $user_id . ") IS NULL,
                0, 1) as exist
            FROM plots ORDER BY plot_id;") or die (DB::error());
        while ($row = DB::fetch_row($q)) {
            $items[] = [
                'plot_id' => $row['plot_id'],
                'exist' => (int) $row['exist']
