...
function paginator($total, $offset, $q, $path, &$out) {
    if ($total > $q) {
        ...
        if ($min < 0) $min = 0;
            ...
            if ($min >= 1) {
                $out .= '<a href="/'.$path.$before_offset.'offset=0">1</a>';
                if ($min != 1) $out .= '&nbsp;&nbsp;...&nbsp;&nbsp;';
            }
        }
        for ($i = $min; $i < $k; $i++) {
            $m = $i*$q + $q;
            if ($m > $total) $m = $total;
            $out .= '<a href="/'.$path.$before_offset.'offset='.($i*$q).'">'.$m/$q.'</a>';
        }
        // # of current page
        ...
        for ($i = $k + 1; $i < $min; $i++) {
            $m = $i * $q + $q;
            if ($m > $total) $m = $total;
            $out .= '<a href="/'.$path.$before_offset.'offset='.($i*$q).'">'.ceil($m/$q).'</a>';
        }
        if ($min * $q < $total) {
            if ($min * $q < $total-$q) $out .= '&nbsp;&nbsp;...&nbsp;&nbsp;';
            $out .= '<a href="/'.$path.$before_offset.'offset='.(($total-1)-($total-1)%$q).'">'.ceil($total/$q).'</a>';
        }
    }
}
